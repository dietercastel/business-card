...
\foreach \i in {0,1} \foreach \j in {0,1,2,3,4} {
   % Comment out the correct line to use either a logo or QR Code
   % \node at (\i*\cardw+0.2\cardw,\j*\cardh+0.5\cardh) {\includegraphics[width=0.2\cardw]{logo}};
   \node at (\i*\cardw+0.2\cardw,\j*\cardh+0.5\cardh){\qrcode[nolinks, height=0.3\cardw]{\TheQRContent}};
% center text
